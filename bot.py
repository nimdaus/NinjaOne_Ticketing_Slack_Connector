...
# ---------------------------------------------------------------------------
# NinjaOne helpers (module-level constants only)
# ---------------------------------------------------------------------------
...
def _register_handlers(bolt_app, tenant: Tenant) -> None:

    # ------------------------------------------------------------------
    # Per-tenant closures — capture `tenant` so handlers are stateless
    # ------------------------------------------------------------------

    async def _lookup_ninja_user(email: str) -> int | None:
        """Resolve a Slack user's email to a NinjaOne end-user ID."""
        if email in tenant.user_cache:
            return tenant.user_cache[email]
        headers = await tenant.ninja_auth.headers(tenant.http_client)
        try:
            resp = await tenant.http_client.get(
                f"{tenant.ninja_auth.get_api_base()}/v2/users",
                headers=headers,
            ...
            for user in resp.json():
                if user.get("email", "").lower() == email.lower():
                    tenant.user_cache[email] = user["id"]
        ...
        tenant.user_cache[email] = None
        return None

    async def _fetch_form_schema(form_id: int) -> dict:
        """Fetch and cache the ticket form schema from NinjaOne."""
        cached = tenant.form_schema_cache.get(form_id)
        ...
        resp.raise_for_status()
        schema = resp.json()
        tenant.form_schema_cache[form_id] = {"schema": schema, "fetched_at": time.time()}
    ...
    async def _create_ninja_ticket(
        ...
        """
        headers = await tenant.ninja_auth.headers(http)
        ...
        resp = await http.post(
            f"{api_base}/v2/ticketing/ticket",
            headers=headers,
            json=payload,
        )
    ...
    async def _prefetch_schemas(entries: list[dict]) -> None:
        ...
        for entry in entries:
            try:
                await _fetch_form_schema(entry["ticketFormId"])
            except Exception as exc:
    ...
    async def _build_form_modal(entry: dict, channel_id: str, cmd: str) -> dict | None:
        ...
        try:
            form_schema = await _fetch_form_schema(entry["ticketFormId"])
        except Exception as exc:
    ...
    async def _process_dynamic_submission(
        ...
        try:
            ...
            form_schema = await _fetch_form_schema(ticket_form_id)
            ...
            try:
                ...
                if email:
                    requester_id = await _lookup_ninja_user(email)
            except Exception as exc:
            ...
            existing = await get_open_ticket_by_dedup_key(dedup_key, db_path=tenant.db_path()) if dedup_key else None
    ...
    async def handle_dynamic_submission(ack, body, client, view):
        ...
        asyncio.create_task(
            _process_dynamic_submission(
                tenant, client, user_id, channel_id, cmd,
...
async def _run_slack_connection(tenant: Tenant) -> None:
    """
    ...
    while True:
        ...
        if not bot_token or not app_token:
            ...
            tenant.reconnect_event.clear()
            try:
                await asyncio.wait_for(tenant.reconnect_event.wait(), timeout=60)
        ...
        try:
            ...
            tenant.slack_client = AsyncWebClient(token=bot_token)
            _register_handlers(bolt_app, tenant)
            ...
            tenant.reconnect_event.clear()
            try:
                await asyncio.wait_for(tenant.reconnect_event.wait(), timeout=30)
            except asyncio.TimeoutError:
                pass
            continue
        ...
        tenant.reconnect_event.clear()
        await tenant.reconnect_event.wait()


async def _supervised_slack(tenant: Tenant) -> None:
    while True:
        try:
            await _run_slack_connection(tenant)
        except Exception:
            logger.exception("Slack connection crashed for tenant=%s — restarting in 30s", tenant.id)
            await asyncio.sleep(30)


async def _supervised_poller(tenant: Tenant) -> None:
    while True:
        try:
            await run_poller(tenant)
        except Exception:
            logger.exception("Poller crashed for tenant=%s — restarting in 30s", tenant.id)
            await asyncio.sleep(30)
...
async def main():
    mgr = TenantManager()
    mgr.load_all()
    if migrated := mgr.migrate_legacy():
        logger.info("Legacy data migrated to tenant %s", migrated.url_secret)
        mgr.save()
    await mgr.start_all()

    for t in mgr.get_all():
        await init_db(db_path=t.db_path())
        asyncio.create_task(_supervised_slack(t), name=f"slack-{t.id}")
        asyncio.create_task(_supervised_poller(t), name=f"poller-{t.id}")

    admin_app.state.tenant_manager = mgr
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(prog="bot.py")
    _sub = _parser.add_subparsers(dest="cmd")
    _ct = _sub.add_parser("create-tenant", help="Provision a new tenant and print its admin URL")
    _ct.add_argument("--name", required=True, help="Human-readable tenant name")
    _args = _parser.parse_args()

    if _args.cmd == "create-tenant":
        _password = _getpass.getpass("Admin password: ")
        _confirm  = _getpass.getpass("Confirm password: ")
        ...
        _mgr = TenantManager()
        _mgr.load_all()
        _rec = _mgr.create(_args.name, _password)
        _mgr.save()
        _base = ADMIN_BASE_URL or "http://localhost:8080"
        print(f"\nTenant '{_rec.name}' created.")
        print(f"Admin URL:  {_base}/{_rec.url_secret}/")
        print(f"Note: restart the process to activate this tenant's Slack connection and poller.")
    else:
        asyncio.run(main())
